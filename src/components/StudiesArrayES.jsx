...
const parseStudiesES = (mdContent) => {
  const studiesES = [];
...
const StudiesArrayES = () => {
  const [studiesES, setStudiesES] = useState([]);

  useEffect(() => {
    fetch("/content/StudiesES.md")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((mdContent) => {
        setStudiesES(parseStudiesES(mdContent));
      })
      .catch((error) => {
