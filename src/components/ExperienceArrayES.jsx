...
const parseExperienceES = (mdContent) => {
  const experienceES = [];
...
const ExperienceArrayES = () => {
  const [experienceES, setExperienceES] = useState([]);

  useEffect(() => {
    fetch("/content/ExperienceES.md")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((mdContent) => {
        setExperienceES(parseExperienceES(mdContent));
      })
      .catch((error) => {
...
export default ExperienceArrayES;
